...
class HypersphericalUniform(distributions.Distribution):
	...
	def log_prob(self, x: torch.Tensor):
		...
		# return torch.ones(bshape + self.batch_shape, device=self.device) * (-self._log_surface_area())
...
if __name__ == '__main__':
	...
	x = uniform_sphere.sample(torch.Size([2]))
	print(x)
	# print(uniform_sphere.entropy())
	print(uniform_sphere.log_prob(x))
